...
- **Backend:** **Firebase** (Auth + Firestore, REST, no SDK). `FirebaseService` is the impl; the old `Supabase*` typealiases were removed. (Supabase + Vercel MCP are connected under the "Trainovations" org, but the owner chose Firebase.)
...
2. **Firebase project** — create it, enable Email/Password auth + Firestore (rules in `SETUP-FIREBASE.md`), paste **Project ID + Web API Key** into `Config/Secrets.xcconfig`, and do the one-time **base-config wiring** step in Xcode (also in `SETUP-FIREBASE.md`). Without that base-config step the keys read nil.
...
2. If the owner has **Firebase**: confirm the base-config wiring, then test sign-up → Firestore write → cross-device restore, and the new Delete Account flow end-to-end.
